...
getTicks largest mostticks =
    let
        mini = largest / mostticks
        magnitude = toFloat <| 10 ^ (floor <| logBase 10 mini / logBase 10 10)
        residual = mini / magnitude
    in
        if residual > 5
        then 10 * magnitude
        else if residual > 2
...
viewLine model =
    let
        ...
        chartH = height / 2 - 40
        noTicks = 8
        noValues = (toFloat << length) model.items
        yVals = List.map .value model.items
        tickIncrement =
            getTicks (Maybe.withDefault 0 <| List.maximum yVals) noTicks
        maxY = tickIncrement * noTicks
        lineStyle = { defaultLine | width = 8, color = lightBlue, join = Smooth }
        yaxis =
            traced
                defaultLine
                (segment (-chartW, -chartH) (-chartW, chartH))
            :: (List.map (\i -> i * tickIncrement) [0..noTicks]
                |> List.map makeYTick
        ...
        val2point i v =
            let xSpacing = (2 * chartW) / noValues
            in
...
makeYTick tVal =
    let l = String.length (toString tVal)
    in
    group
